...
class MainTestNew {
    ...
    class UkrainianLanguageTest {
        ...
        @DisplayName("[ENCRYPT] Simple letters decryption")
        @ParameterizedTest
        @CsvSource({"Б, 1, А",
                "б, 1, а",
                "Я, 32, А",
    ...
    private Path execute(String command, Path inputFilePath, Integer key) {
        ...
        params[1] = command;
        params[2] = "-f";
        params[3] = inputFilePath.toString();
        params[4] = "-k";
        params[5] = String.valueOf(120);
        params[6] = "-m";
        params[7] = "CONSOLE";
    ...
    private Path getCreatedFile(List<Path> filesBefore)  {
        List<Path> filesAfter = getFiles(tempDir);

        if (filesAfter.size() > filesBefore.size()){
            return filesAfter.stream().filter(file -> !filesBefore.contains(file)).findFirst().get();
        } else {
